...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    const nuevoUsuario = await Usuario.create({ nombre, email, password: hashedPassword, rol: rol || 'usuario' });

    const token = jwt.sign({ id: nuevoUsuario.id, rol: nuevoUsuario.rol }, jwtSecret, { expiresIn: process.env.JWT_EXPIRES || '1h' });
    res.json({ message: 'Usuario registrado correctamente.', usuario: nuevoUsuario, token });
  } catch (error) {
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: usuario.id, nombre: usuario.nombre, rol: usuario.rol }, jwtSecret, { expiresIn: process.env.JWT_EXPIRES || '1h' });
    res.status(200).json({ message: 'Login exitoso', token, usuario: { id: usuario.id, nombre: usuario.nombre, rol: usuario.rol } });
  } catch (error) {
...
app.get('/dashboard', authenticateToken, (req, res) => {
  if (req.usuario.rol !== 'admin') return res.status(403).json({ message: 'Solo para administradores.' });
...
app.listen(PORT, "0.0.0.0", () => console.log(`Servidor escuchando en Railway en el puerto ${PORT}`));
/*const express = require('express');
